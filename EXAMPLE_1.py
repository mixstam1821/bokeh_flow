

from real_world_coastlines_example import *

def example_north_atlantic():
    """Default: North Atlantic with Gulf Stream"""
    return {
        'lon_min': -100, 'lon_max': 10,
        'lat_min': 20, 'lat_max': 70,
        'width': 1000, 'height': 700,
        'title': '🌊 North Atlantic Ocean',
        'description': 'Gulf Stream, North Atlantic Drift, Labrador Current'
    }

def example_mediterranean():
    """Mediterranean Sea"""
    return {
        'lon_min': -10, 'lon_max': 40,
        'lat_min': 30, 'lat_max': 46,
        'width': 1200, 'height': 500,
        'title': '🏛️ Mediterranean Sea',
        'description': 'Complex circulation with strait connections'
    }

def example_caribbean():
    """Caribbean Sea and Gulf of Mexico"""
    return {
        'lon_min': -100, 'lon_max': -55,
        'lat_min': 10, 'lat_max': 32,
        'width': 1100, 'height': 650,
        'title': '🏝️ Caribbean Sea',
        'description': 'Caribbean Current and Loop Current'
    }

def example_pacific_northwest():
    """Pacific Northwest (Alaska to California)"""
    return {
        'lon_min': -140, 'lon_max': -115,
        'lat_min': 30, 'lat_max': 60,
        'width': 700, 'height': 900,
        'title': '🌲 Pacific Northwest',
        'description': 'Alaska Current and California Current'
    }

def example_nordic_seas():
    """Norwegian and Greenland Seas"""
    return {
        'lon_min': -40, 'lon_max': 30,
        'lat_min': 60, 'lat_max': 80,
        'width': 1000, 'height': 650,
        'title': '❄️ Nordic Seas',
        'description': 'Norwegian Atlantic Current and East Greenland Current'
    }

def example_south_china_sea():
    """South China Sea and Philippines"""
    return {
        'lon_min': 100, 'lon_max': 130,
        'lat_min': 0, 'lat_max': 25,
        'width': 800, 'height': 800,
        'title': '🌏 South China Sea',
        'description': 'Monsoon-driven circulation'
    }

def example_australian_coast():
    """Eastern Australia"""
    return {
        'lon_min': 140, 'lon_max': 165,
        'lat_min': -45, 'lat_max': -10,
        'width': 700, 'height': 1000,
        'title': '🦘 Australian Coast',
        'description': 'East Australian Current'
    }

def example_benguela_current():
    """Southwest Africa - Benguela Current System"""
    return {
        'lon_min': -5, 'lon_max': 25,
        'lat_min': -40, 'lat_max': -10,
        'width': 800, 'height': 900,
        'title': '🐧 Benguela Current',
        'description': 'Cold upwelling system off African coast'
    }

def generate_regional_currents(config, width, height):
    """
    Generate currents based on region type.
    This is a simplified version - you can enhance it with region-specific patterns.
    """
    lon_min = config['lon_min']
    lon_max = config['lon_max']
    lat_min = config['lat_min']
    lat_max = config['lat_max']
    
    # Use the Atlantic current generator as a base
    # In practice, you'd customize this for each region
    return generate_currents(
        lon_min=lon_min, lon_max=lon_max,
        lat_min=lat_min, lat_max=lat_max,
        width=width, height=height
    )

def create_visualization(region_config):
    """Create visualization for a specific region"""
    
    if not CARTOPY_AVAILABLE:
        print("\n⚠️  ERROR: Cartopy is required")
        print("Install with: conda install -c conda-forge cartopy")
        return
    
    config = region_config()
    
    print("\n" + "="*70)
    print(f"🌍 Creating: {config['title']}")
    print("="*70)
    print(f"Region: {config['lat_min']}°N to {config['lat_max']}°N")
    print(f"        {config['lon_min']}°E to {config['lon_max']}°E")
    print(f"Features: {config['description']}")
    print("="*70)
    
    # Create background
    print("Loading coastline data...")
    bg_image = create_cartopy_background(
        lon_min=config['lon_min'],
        lon_max=config['lon_max'],
        lat_min=config['lat_min'],
        lat_max=config['lat_max'],
        width=config['width'],
        height=config['height'],
        dpi=100
    )
    print("✓ Coastlines loaded")
    
    # Generate currents
    print("Generating ocean currents...")
    x, y, dx, dy, mag = generate_regional_currents(
        config, config['width'], config['height']
    )
    print("✓ Currents generated")
    
    # Create flow field
    flow = FlowFieldWithBackground(
        width=config['width'],
        height=config['height'],
        x_coords=x, y_coords=y,
        dx_values=dx, dy_values=dy,
        magnitudes=mag,
        particle_count=2000,
        particle_size=1,
        particle_life=120,
        flow_strength=4.0,
        particle_trail=True,
        background_image=bg_image,
        background_alpha=1.0,
        color_scheme='lightblue',
        show_vectors=False,
        animate=True
    )
    
    # Create controls
    controls = create_controls(flow, config['width'])
    
    # Layout
    layout = row( flow, controls, sizing_mode="fixed")
    
    # Generate filename from title
    filename = config['title'].lower()
    filename = ''.join(c if c.isalnum() else '_' for c in filename)
    filename = f"{filename}.html"
    
    output_file(filename)
    save(layout)
    
    print(f"\n✅ Created: {filename}")
    print("="*70)
    
    return filename

# ============================================
# MAIN - Choose your region!
# ============================================

if __name__ == "__main__":

    result = create_visualization(example_mediterranean)
    # result = create_visualization(example_caribbean)
    # result = create_visualization(example_pacific_northwest)
    # result = create_visualization(example_nordic_seas)
    # result = create_visualization(example_south_china_sea)
    # result = create_visualization(example_australian_coast)
    # result = create_visualization(example_benguela_current)
